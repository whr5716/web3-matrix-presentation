...
describe('Web3 Matrix Presentation', () => {
  ...
  describe('Asset URLs', () => {
    it('should have valid asset URLs', () => {
      expect(web3MatrixPresentation.presenter.avatarUrl).toMatch(/^\/\w+/);
      expect(web3MatrixPresentation.backgroundMusic?.url).toMatch(/\.mp3$/);
    });

    it('should have all slides with valid background colors', () => {
      web3MatrixPresentation.slides.forEach((slide) => {
        expect(slide.backgroundColor).toMatch(/^#[0-9a-f]{6}$/i);
      });
    });
  });
});
